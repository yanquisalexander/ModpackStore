...
export const PublisherSubscriptionView: React.FC = () => {
    ...
    const handleCancelSubscription = async () => {
        ...
        if (!confirm('¿Estás seguro de que deseas cancelar tu suscripción? Perderás el acceso a las funciones premium al final de tu periodo de facturación.')) {
            return;
        }
                            ...
                            <Card className={`h-full flex flex-col relative overflow-hidden transition-all hover:shadow-xl hover:scale-[1.02] duration-300 ${isCurrent ? 'border-primary ring-2 ring-primary/20' : 'border-border'}`}>
                                ...
                                <CardContent className="flex-1 space-y-4">
                                    <div className="space-y-2 text-sm">
                                        {PLAN_FEATURES.map((feature) => {
                                            ...
                                            if (typeof val === 'boolean') displayVal = 'Incluido';
                                ...
                                <CardFooter>
                                    <Button
                                        ...
                                        disabled={isCurrent || (subscription?.tier !== SubscriptionTier.FREE)} // Simplify logic: can only upgrade if Free? Or upgrade any? Assuming upgrade from free/basic
    ...
    const handlePayPalPayment = async () => {
        ...
        try {
            // Create PayPal order
            const orderId = await subscriptionService.createPayPalOrder(
                publisherId,
                selectedTier,
                sessionTokens.accessToken
            );

            // Get PayPal approval URL from the order
            const approvalUrl = await subscriptionService.getPayPalApprovalUrl(orderId, sessionTokens.accessToken);

            // Open PayPal in new window
            const paypalWindow = window.open(approvalUrl, 'paypal-payment', 'width=800,height=600,scrollbars=yes,resizable=yes');

            if (!paypalWindow) {
                toast.error('Por favor, permite las ventanas emergentes para completar el pago');
                setIsProcessing(false);
                return;
            }
            ...
            const checkPayment = async () => {
                try {
                    const result = await subscriptionService.checkPaymentStatus(orderId, sessionTokens.accessToken);
            ...
            // Close the PayPal window and clear interval on unload
            paypalWindow.onunload = () => {
            ...
            >
                ...
                {loading ? (
                    ...
                    <div>
                        ...
                        {subscription && subscription.tier !== SubscriptionTier.FREE && (
                            <div className="mt-8">
                                ...
                                <p className="text-sm text-muted-foreground mb-4">
                                    Selecciona un plan diferente para actualizar o reducir tu suscripción.
                                </p>
                                {renderPricingCards()}
                            </div>
                        )}

                        {/* Payment Method Section - Only show if not Free Tier */}
                        {subscription && subscription.tier !== SubscriptionTier.FREE && (
                            <div className="mt-8">
                                <h3 className="text-xl font-semibold mb-4">
                                    Método de Pago
                                ...
                                <p className="text-sm text-muted-foreground mb-4">
                                    Tu método de pago actual es {subscription.paymentMethod}. Puedes cambiar tu método de pago en la configuración de tu cuenta.
                                </p>
                                <Button
                                    variant="default"
                        ...
                        {/* Free Tier Upgrade Prompt - Only show if current tier is Free */}
                        {subscription && subscription.tier === SubscriptionTier.FREE && (
                            <div className="mt-8 p-4 rounded-lg bg-primary/10 border border-primary">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-lg font-semibold">
                                        ¡Mejora a un Plan Premium!
                                    </h3>
                                </div>
                                <p className="text-sm text-muted-foreground mt-2">
                                    Disfruta de beneficios adicionales como modpacks ilimitados, mayor almacenamiento y soporte prioritario.
                                </p>
                                <Button
                                    variant="default"
                                    className="mt-4"
                                    onClick={() => setSelectedTier(SubscriptionTier.PREMIUM)}
                                >

...
async fn download_missing_assets(
    ...
    log::info!(
        "Se encontraron {} assets faltantes. Iniciando descarga en lote...",
        missing_assets.len()
    );
    ...
    // Prepare an Arc clone of the instance so the progress callback can be 'static
    let instance_arc = Arc::new(instance.clone());

    // Use batch download with progress reporting
    download_manager
        .download_files_parallel_with_progress(files_to_download, {
            ...
            move |current, total, message| {
                ...
                emit_status_with_stage(&*instance_arc, "instance-downloading-assets", &stage);

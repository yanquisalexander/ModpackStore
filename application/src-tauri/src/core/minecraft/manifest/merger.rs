...
impl ManifestMerger {
    ...
    fn merge_libraries(result: &mut Value, vanilla: &Value, forge: &Value) {
            ...
            for lib in arr {
                if let Some((name, ga, version, _, classifier)) = Self::extract_lib_info(lib) {
                    ...
                    if let Some(existing_lib) = merged_libs.get_mut(&key) {
                        // La clave ya existe, tenemos un conflicto de versión DENTRO del manifiesto de vanilla.
                        // Esto es común con LWJGL (versiones distintas para diferentes SO).
                        // Nos quedamos con la versión más reciente.
                        if let Some((_, _, existing_ver, _, _)) =
                            Self::extract_lib_info(existing_lib)
                        {
                            ...
                            if Self::is_version_newer(new_ver_str, existing_ver_str) {
                                log::debug!(
                                    "Internal conflict resolved (vanilla): replacing {} with {}",
                                    existing_lib
                                        .get("name")
                                ...
                                *existing_lib = lib.clone();
                            }
                        }
                    } else {
                        // La clave no existe, la insertamos normalmente.
        ...
        // --- BUCLE DE FORGE (SIN CAMBIOS EN SU LÓGICA ORIGINAL) ---
                ...
                if let Some((name, ga, fver, _, classifier)) = Self::extract_lib_info(forge_lib) {
                    ...
                    if let Some(existing_lib) = merged_libs.get_mut(&key) {
                        if let Some((_, _, vver, _, _)) = Self::extract_lib_info(existing_lib) {
                            let new_ver_str = fver.as_deref().unwrap_or_default();
                            let old_ver_str = vver.as_deref().unwrap_or_default();

                            if Self::is_version_newer(new_ver_str, old_ver_str) {
                                log::info!(
                            ...
                            } else if new_ver_str == old_ver_str {
                                log::debug!(
    ...
    fn build_lib_key(_ga: &str, _version: &Option<String>, classifier: &Option<String>) -> String {
        let mut key = _ga.to_string();

...
export class ModpackFileUploadService {
  ...
  constructor(region: string, bucketName: string, endpoint?: string, accessKeyId?: string, secretAccessKey?: string) {
    this.bucketName = bucketName;
   ...
   */
  private async saveFileToStorage(key: string, fileBuffer: Buffer, contentType: string, metadata?: Record<string, string>): Promise<void> {
    await this.s3Client.send(new PutObjectCommand({
      Bucket: this.bucketName,
      Key: key,
      Body: fileBuffer,

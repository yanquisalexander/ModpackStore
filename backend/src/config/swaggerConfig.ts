...
const swaggerDefinition = {
  ...
  components: {
    securitySchemes: {
      bearerAuth: { // Defines 'bearerAuth' for use in endpoint security
        type: 'http',
        scheme: 'bearer',
    ...
    schemas: {
      ...
      JsonApiRelationshipToMany: {
        ...
        properties: {
          ...
          meta: { // For pagination within relationships
            type: 'object',
            properties: {
              count: { type: 'integer' }
            }
      ...
      TokenResource: { // Used for /auth/discord/callback and /auth/refresh
        type: 'object',
        required: ['type', 'id', 'attributes'],
        properties: {
      ...
      NewPublisherAttributes: { // For creating a publisher
        type: 'object',
        required: ['name', 'slug'],
        properties: {
          name: { type: 'string', example: 'Awesome Modding Inc.' },
      ...
      UpdatePublisherAttributes: { // For updating a publisher
        type: 'object',
      ...
      NewModpackAttributes: { // For creating a modpack
        type: 'object',
      ...
      UpdateModpackAttributes: { // For updating a modpack
        type: 'object',
        properties: { // All optional for PATCH
          name: { type: 'string', minLength: 3, maxLength: 64 },
      ...
      NewModpackVersionAttributes: { // For creating a version
        type: 'object',
        required: ['version'],
        properties: {
      ...
      UpdateModpackVersionAttributes: { // For updating a version
        type: 'object',
        properties: { // All optional for PATCH
  ...
  tags: [ // Optional: Define tags for grouping routes in Swagger UI
